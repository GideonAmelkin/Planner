...
export default function DailyNotes({ dateISO, notes, onChange, onDropTask }) {
  ...
  const handleDragOver = (e) => {
    if (Array.from(e.dataTransfer.types).includes('application/x-planner-task')) {
      e.preventDefault();
      e.dataTransfer.dropEffect = 'move';
  ...
  const handleDrop = (e) => {
    const raw = e.dataTransfer.getData('application/x-planner-task');
    setDragOver(false);
    if (!raw) return;
    e.preventDefault();
    try {
      const payload = JSON.parse(raw);
      onDropTask && onDropTask(payload);
    } catch (_) {}
  };
